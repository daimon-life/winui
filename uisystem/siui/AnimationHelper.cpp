...
const TImageInfo* CAnimation::LoadGIF(LPCTSTR bitmap, LPCTSTR type/* = NULL*/, DWORD mask/* = 0*/)
{
    ...
    if( type != NULL && isdigit(*bitmap) )
    {
        ...
        HRSRC hPicture = ::FindResource(CPaintManagerUI::GetGlobalResourceDll(), MAKEINTRESOURCE(iIndex), type);
        if (!hPicture)
            return NULL;
        HGLOBAL hResData;
        if (!(hResData = ::LoadResource(CPaintManagerUI::GetGlobalResourceDll(), hPicture)))
        {
            ::FreeResource(hPicture);
            return NULL;
        };
        dwSize = ::SizeofResource(CPaintManagerUI::GetGlobalResourceDll(),hPicture);
    ...
    else
    {
        ...
        if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
			...
			if(INVALID_HANDLE_VALUE == hFile)//去查看一下全路径的
				hFile = ::CreateFile(bitmap, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, \
