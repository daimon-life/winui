...
namespace DuiLib {
    ...
    HMODULE CPaintManagerUI::m_hGlobalResourceModule = NULL;
	CStdString CPaintManagerUI::m_pStrResourcePath;
	CStdString CPaintManagerUI::m_pStrResourceZip;
	CStdString CPaintManagerUI::m_pStrXMLResourceZip;
	bool CPaintManagerUI::m_bCachedResourceZip = false;
	HANDLE CPaintManagerUI::m_hResourceZip = NULL;
	bool CPaintManagerUI::m_bCachedXMLResourceZip = false;
	HANDLE CPaintManagerUI::m_hXMLResourceZip = NULL;
	...
	bool CPaintManagerUI::EnableDropTarget(void)
	{
		if( m_pDragDrop == NULL )
		{
			if( m_pDragDrop = new IDragDropTarget(m_hWndPaint) )
			{
				::RegisterDragDrop(m_hWndPaint,m_pDragDrop);
				return true;
			}
			else
				return false;
	...
	void CPaintManagerUI::RemoveDropUI( CControlUI* pUI )
	{
		if( m_pDragDrop )
			m_pDragDrop->RemoveDropUI(pUI);
	}
	...
	const CStdString& CPaintManagerUI::GetXMLResourceZip()
	{
		return m_pStrXMLResourceZip;
	}

	bool CPaintManagerUI::IsCachedXMLResourceZip()
	{
		return m_bCachedXMLResourceZip;
	}

	HANDLE CPaintManagerUI::GetXMLResourceZipHandle()
	{
		return m_hXMLResourceZip;
	}
    ...
    void CPaintManagerUI::SetGlobalResourceDll(HMODULE hModule)
    {
        m_hGlobalResourceModule = hModule;
    }

    HMODULE CPaintManagerUI::GetGlobalResourceDll()
    {
        if (m_hGlobalResourceModule == NULL)
            return (HMODULE)m_hInstance;

        return m_hGlobalResourceModule;
    }
	...
	void CPaintManagerUI::SetXMLResourceZip(LPVOID pVoid, unsigned int len)
	{
		if( m_pStrXMLResourceZip == _T("membuffer") ) return;
		if( m_bCachedXMLResourceZip && m_hXMLResourceZip != NULL ) {
			CloseZip((HZIP)m_hXMLResourceZip);
			m_hXMLResourceZip = NULL;
		}
		m_pStrXMLResourceZip = _T("membuffer");
		m_bCachedXMLResourceZip = true;
		if( m_bCachedXMLResourceZip ) 
			m_hXMLResourceZip = (HANDLE)OpenZip(pVoid, len, 3);
	}

	void CPaintManagerUI::SetXMLResourceZip(LPCTSTR pStrPath, bool bCachedResourceZip)
	{
		if( m_pStrXMLResourceZip == pStrPath && m_bCachedXMLResourceZip == bCachedResourceZip ) return;
		if( m_bCachedXMLResourceZip && m_hXMLResourceZip != NULL ) {
			CloseZip((HZIP)m_hXMLResourceZip);
			m_hXMLResourceZip = NULL;
		}
		m_pStrXMLResourceZip = pStrPath;
		m_bCachedXMLResourceZip = bCachedResourceZip;
		if( m_bCachedXMLResourceZip ) {
			CStdString sFile = CPaintManagerUI::GetResourcePath();
			sFile += CPaintManagerUI::GetXMLResourceZip();
			m_hXMLResourceZip = (HANDLE)OpenZip((void*)sFile.GetData(), 0, 2);
		}
	}
    ...
    HMODULE CPaintManagerUI::GetResourceDll()
    {
        return m_hResourceModule;
    }

    void CPaintManagerUI::SetResourceDll(HMODULE hModule)
    {
	...
	case WM_PAINT:
		{
            ...
            if (!::GetClientRect(m_hWndPaint, &rcClient)) return false;
			...
			if( m_bAlphaBackground)
			{
				...
				if( m_bOffscreenPaint && m_hbmpBackground == NULL )
				{
					...
					bitmapinfo.bmiHeader.biHeight = rcClient.bottom - rcClient.top;
					bitmapinfo.bmiHeader.biWidth = rcClient.right - rcClient.left;
					...
					m_hbmpBackground=::CreateDIBSection (m_hDcBackground,&bitmapinfo, 0,NULL, 0, 0);
			...
			if( m_bUpdateNeeded )
			{
				...
				if( !::IsRectEmpty(&rcClient) )
				{
					if( m_pRoot->IsUpdateNeeded() )
					{
                        ...
                        if (m_hDcBGMemory)
                        {
                            ::SelectObject(m_hDcBGMemory, m_hBGOldBitmap);
                            ::DeleteObject(m_hBGOldBitmap);
                            ::DeleteObject(m_hBGBitmap);
                            ::DeleteDC(m_hDcBGMemory);
                            m_hDcBGMemory = NULL;
                        }
					}
					else
            ...
            ::BeginPaint(m_hWndPaint, &ps);
            /*
            if (m_hDcBGMemory == NULL)
            {
                m_hDcBGMemory = CreateCompatibleDC(m_hDcPaint);
                // 创建背景画布
                //创建背景画布 
                BLENDFUNCTION bf = { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };
                BITMAPINFOHEADER stBmpInfoHeader = { 0 };
                PVOID pvBits = NULL;
                int nBytesPerLine = (((rcClient.right - rcClient.left) * 32 + 31) & (~31)) >> 3;
                stBmpInfoHeader.biSize = sizeof(BITMAPINFOHEADER);
                stBmpInfoHeader.biWidth = (rcClient.right - rcClient.left);
                stBmpInfoHeader.biHeight = (rcClient.bottom - rcClient.top);
                stBmpInfoHeader.biPlanes = 1;
                stBmpInfoHeader.biBitCount = 32;
                stBmpInfoHeader.biCompression = BI_RGB;
                stBmpInfoHeader.biClrUsed = 0;
                stBmpInfoHeader.biSizeImage = nBytesPerLine * (rcClient.bottom - rcClient.top);
                m_hBGBitmap = ::CreateDIBSection(NULL, (PBITMAPINFO)&stBmpInfoHeader, DIB_RGB_COLORS, &pvBits, NULL, 0);
                ASSERT(m_hBGBitmap != NULL);
                m_hBGOldBitmap = ::SelectObject(m_hDcBGMemory, m_hBGBitmap);
                // 开始绘制背景图
                m_pRoot->DoPaint(m_hDcBGMemory, rcClient);
                for (int i = 0; i < m_aPostPaintControls.GetSize(); i++) {
                    CControlUI* pPostPaintControl = static_cast<CControlUI*>(m_aPostPaintControls[i]);
                    pPostPaintControl->DoPostPaint(m_hDcBGMemory, rcClient);
                }
                ::BitBlt(ps.hdc, rcClient.left, rcClient.top, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top,
                    m_hDcBGMemory, rcClient.left, rcClient.top, SRCCOPY);
            }
            else
            */
            {
                BLENDFUNCTION bf = { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };
                HDC hDcEnd = CreateCompatibleDC(m_hDcPaint); //新建兼容DC
                PVOID pvBits2 = NULL;
                HBITMAP hMemBitmap = NULL;
                BITMAPINFOHEADER stBmpInfoHeader2 = { 0 };
                int nBytesPerLine2 = (((rcClient.right - rcClient.left) * 32 + 31) & (~31)) >> 3;
                stBmpInfoHeader2.biSize = sizeof(BITMAPINFOHEADER);
                stBmpInfoHeader2.biWidth = (rcClient.right - rcClient.left);
                stBmpInfoHeader2.biHeight = (rcClient.bottom - rcClient.top);
                ...
                stBmpInfoHeader2.biSizeImage = nBytesPerLine2 * (rcClient.bottom - rcClient.top);
                hMemBitmap = ::CreateDIBSection(NULL, (PBITMAPINFO)&stBmpInfoHeader2, DIB_RGB_COLORS, &pvBits2, NULL, 0);//新建画布 
                ASSERT(hMemBitmap != NULL);
                HGDIOBJ hEndOldBitmap = SelectObject(hDcEnd, hMemBitmap);
                /*
                ::BitBlt(hDcEnd, rcClient.left, rcClient.top, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top,
                    m_hDcBGMemory, rcClient.left, rcClient.top, SRCCOPY);
                    */
                // 开始绘图
                m_pRoot->DoPaint(hDcEnd, rcClient);
                for (int i = 0; i < m_aPostPaintControls.GetSize(); i++) {
                    CControlUI* pPostPaintControl = static_cast<CControlUI*>(m_aPostPaintControls[i]);
                    pPostPaintControl->DoPostPaint(hDcEnd, rcClient);
                }
                ::BitBlt(ps.hdc, rcClient.left, rcClient.top, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top,
                    hDcEnd, rcClient.left, rcClient.top, SRCCOPY);
                ::SelectObject(hDcEnd, hEndOldBitmap);
                ::DeleteObject(hMemBitmap);
                ::DeleteObject(hEndOldBitmap);
                ::DeleteDC(hDcEnd);
            }
            ::EndPaint(m_hWndPaint, &ps);
#else
	...
	case WM_MOUSEHOVER:
		{
			...
			if (pHover->IsEnabled())
			{
                ...
                {
                    RECT stTipMargin = { 5, 2, 5, 2 };

					m_hwndTooltip = ::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, m_hWndPaint, NULL, m_hInstance, NULL);
					::SendMessage(m_hwndTooltip, TTM_ADDTOOL, 0, (LPARAM) &m_ToolTip);
                    ::SendMessage(m_hwndTooltip, TTM_SETMARGIN, 0, (LPARAM)&stTipMargin);
                    ::SendMessage(m_hwndTooltip, TTM_SETTIPBKCOLOR, (WPARAM)RGB(255, 255, 255), 0);
				}
	...
	case WM_LBUTTONDOWN:
		{
// 			HIMC hImc = ImmGetContext(m_hWndPaint);
// 			ImmAssociateContext(m_hWndPaint, NULL);
...
// 			if (_tcsicmp(pControl->GetClass(),_T("RichEditUI")) == 0)
// 			{
// 				ImmAssociateContext(m_hWndPaint, hImc);
// 			}
		}
		break;
	...
	case WM_SETCURSOR:
		{
			...
			CControlUI* pControl = FindControl(pt);
			if( pControl == NULL ) break;
			// 			if (pControl->GetInterface(_T("Button")) != NULL)
			// 			{
			// 				int a = 0;
			// 			}
	...
	void CPaintManagerUI::Invalidate(RECT& rcItem,BOOL b)
	{
		::InvalidateRect(m_hWndPaint, &rcItem, b);
	}
	...
	void CPaintManagerUI::SetFocus(CControlUI* pControl)
	{
        ...
        if ((hFocusWnd != m_hWndPaint) && pControl)
        {
            ::SetFocus(m_hWndPaint);
        }
        if (m_pFocus)
        ...
        if (pControl && (pControl->GetManager() == this) && pControl->IsVisible() && pControl->IsEnabled())
        {
            memset(&event, 0, sizeof(event));
	...
	{
		return CImgObjectMgr::GetImgMgr()->GetImage(this,bitmap,type,mask);
		TImageInfo* data = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
		if( !data ) {
	...
	const TImageInfo* CPaintManagerUI::GetGrayImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
	{
		return CImgObjectMgr::GetImgMgr()->GetGrayImage(this,bitmap,type,mask);
		...
		if (data->hGrayBitmap == NULL)
		{
			data->hGrayBitmap = CRenderEngine::ConverColToGray(data->hBitmap);
		}
	...
	bool CPaintManagerUI::RemoveImage(LPCTSTR bitmap)
	{
		...
		return CImgObjectMgr::GetImgMgr()->RemoveImage(this,bitmap);
	...
	{
		CImgObjectMgr::GetImgMgr()->ReleaseImage(this);
		TImageInfo* data;
	...
	void CPaintManagerUI::TRACEINFO()
	{
		TRACE(_T("此CLASS名字是%s\r\n"),m_ClassName);
	}
