...
err:
	...
	HRESULT CTxtWinHost::TxNotify(DWORD iNotify, void *pv)
	{
		...
		if( iNotify == EN_LINK )
		{
			...
			//return S_OK; // XNRicheditEx类中需要处理文字链接
		}
		...
		bool OnOleObject(IOleObject* pObject,const CLSID& pIID,long lPos)
		{
			...
			if( pIID == Guid_ImageObject )
			{
				...
				if( pImageOle )
				{
					...
					m_pUi->SetSel(lPos,lPos+1);			//OLE对象占一个字符
					m_pUi->ReplaceSel(_T("   "),true);	//此操作相当于删除OLE对象
				}
			}
	...
	BOOL CRichEditUI::InsertImage(LPCTSTR szFilePath,long lPos,bool bClick, int faceid)
	{
		...
		CreateNewGUID(&pNewGuid);
		// 	if( lPos == -1 )
		// 		lPos = getEndCharPos();//这个不怎么准确。还是别用了
...
// 			}
	...
	{
		//OleLib::CImageOleObject* pimageobject = static_cast<OleLib::CImageOleObject*>(lpdata); // 类型不安全
		//return pimageobject->getFilePath();// 参数没有检查

		OleLib::CImageOleObject* pimageobject = dynamic_cast<OleLib::CImageOleObject*>(lpdata);// 准确做法应该是完成IMsgImage接口，然后再Query
	...
	{
		//OleLib::CImageOleObject* pimageobject = static_cast<OleLib::CImageOleObject*>(lpdata); // 类型不安全
		//return pimageobject->GetFaceID(); // 参数没有检查

		OleLib::CImageOleObject* pimageobject = dynamic_cast<OleLib::CImageOleObject*>(lpdata);// 准确做法应该是完成IMsgSysFace接口，然后再Query
		if ( pimageobject != NULL )
