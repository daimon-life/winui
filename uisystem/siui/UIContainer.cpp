...
void CContainerUI::RemoveAll()
{
    ...
    for(int i = 0; m_bAutoDestroy && i < m_items.GetSize(); i++)
    {
        pControlUI = static_cast<CControlUI *>(m_items[i]);
        if (pControlUI)
        {
            if (m_bDelayedDestroy && m_pManager)
                m_pManager->AddDelayedCleanup(pControlUI);
            else
                delete pControlUI;
        }
    }
...
CControlUI* CContainerUI::FindControl(FINDCONTROLPROC Proc, LPVOID pData, UINT uFlags)
{
    ...
    if( (uFlags & UIFIND_TOP_FIRST) != 0 ) {
        ...
        if (!m_items.IsEmpty())
        {
            int i = 0, count = m_items.GetSize();
            for (i = 0; i < count; i++)
            {
                CControlUI* pControl = static_cast<CControlUI*>(m_items[i])->FindControl(Proc, pData, uFlags);
                if (pControl != NULL) {
                    if ((uFlags & UIFIND_HITTEST) != 0 && !pControl->IsFloat() && !::PtInRect(&rc, *(static_cast<LPPOINT>(pData))))
                        continue;
                    else
...
void CVerticalLayoutUI::SetPos(RECT rc)
{
    ...
    int cyFixed = 0;
    int nEstimateNum = 0;
	int nTotalProportion = 0; //所有的占的比例
    for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
        ...
        if( sz.cy == 0 ) {
            nAdjustables++;
			nTotalProportion += pControl->GetProportion();
        }
        else {
    ...
    int cyNeeded = 0;
    int cyExpand = 0;
    if( nAdjustables > 0 ) cyExpand = MAX(0, (szAvailable.cy - cyFixed) / nTotalProportion);
    ...
    for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
        ...
        if( sz.cy == 0 ) {
            iAdjustable++;
            sz.cy = cyExpand * pControl->GetProportion();
...
void CHorizontalLayoutUI::SetPos(RECT rc)
{
    ...
    int cxExpand = 0;
    if( nAdjustables > 0 ) cxExpand = MAX(0, (szAvailable.cx - cxFixed) / nTotalProportion);
    // Position the elements
    SIZE szRemaining = szAvailable;
    int iPosX = rc.left;
    if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible() ) {
    ...
    for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
        ...
        if( sz.cx == 0 ) {
            iAdjustable++;
            sz.cx = cxExpand * pControl->GetProportion();
            // Distribute remaining to last element (usually round-off left-overs)
            if( iAdjustable == nAdjustables ) {
...
{
	return SelectItem(GetItemIndex(pControl));
}
