...
{
public:
	explicit StreamOutResult( const DuiLib::duistring* szMemory ) : m_bClear(false)
	{
		m_nObjectNum = -1;
		m_Images	 = 0;
		m_memory = const_cast<DuiLib::duistring*>(szMemory);
	}
	StreamOutResult( ) : m_bClear(true)
	{
		m_memory = new DuiLib::duistring;
	}
	~StreamOutResult()
	{
		m_nObjectNum = -1;
	...
	std::map<char*,DuiLib::duistring>*	m_Images;
private:
	int								m_nObjectNum;
	DuiLib::duistring*					m_memory;
	bool							m_bClear;
};
