...
namespace DuiLib
{
    ...
    void CComboWnd::Notify(TNotifyUI& msg)
    {
        if (msg.sType == _T("click"))
        {
            ...
            TRACE(_T("itemclick"));
            CControlUI* ptemp = msg.pSender;
            while (ptemp->GetInterface(_T("ListItem")) == NULL)
            {
            ...
            if (ptemp)
            {
                if (m_pOwner) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("ComboItemClick"), ((CListContainerElementUI *)ptemp)->GetIndex(), (LPARAM)msg.pSender, true);
            }
        }
    }
    ...
    LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        else if (uMsg == WM_LBUTTONUP)
        {
            ...
            // 				event.wParam=((CListContainerElementUI *)ptemp)->GetIndex();
            // 				event.lParam= (LPARAM)pControl;
            // 				m_pOwner->SetIsSendItemClick();
            // 				//m_pOwner->DoEvent(event);
            // 
            // 				//::PostMessage(m_pOwner->GetManager()->GetPaintWindow(),UMSG_DELUSERNAME,((CListContainerElementUI *)ptemp)->GetIndex(),(LPARAM)pControl);
            // 
            // 				//m_pOwner->GetManager()->SendNotify(pControl,_T("ComboItemClick"),((CListContainerElementUI *)ptemp)->GetIndex());
            // 
            // 				//m_pOwner->GetManager()->SendNotify(m_pOwner,_T("ComboItemClick"),((CListContainerElementUI *)ptemp)->GetIndex(),(LPARAM)pControl);
            // 			}
    ...
    CComboUI::CComboUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0), m_SendItemClick(false)
    {
    ...
    void CComboUI::DoSubEvent(CControlUI* pcontrol, TSUBEventUI& event)
    {
    }
    ...
    bool CComboUI::IsMultipleSele() const
    {
        return false;
    }

    bool CComboUI::ItemIsFolder(int iIndex)
    {
        return false;
    }

    bool CComboUI::ItemIsSelect(int iIndex)
    {
        return false;
    }
    ...
    bool CComboUI::SelectMultipleItem(int iIndex, bool bTakeFocus)
    {
        return true;
    }

    bool CComboUI::DelSelectMultipleItem(int iIndex, bool bTakeFocus)
    {
        return true;
    }

    void CComboUI::GetMultipleSeleList(vector<DWORD>& indexs)
    {
    ...
    bool CComboUI::AddStringItem(LPCTSTR lpItemValue)
    {
        bool bRet = false;

        if (lpItemValue)
        {
			RECT rcTextPadding = { m_ListInfo.rcTextPadding.left, m_ListInfo.rcTextPadding.top, m_ListInfo.rcTextPadding.right, m_ListInfo.rcTextPadding.bottom };
			// RECT rcTextPadding = { 5, 0, 0, 0 }; 原来的 2016/10/27
            CListLabelElementUI *pNewNode = new (std::nothrow) CListLabelElementUI();
            if (pNewNode)
            {
                pNewNode->SetFixedHeight(m_nStringItemHeight);
                pNewNode->SetText(lpItemValue);
                pNewNode->SetTextPadding(rcTextPadding);
                bRet = Add(pNewNode);
                ASSERT(bRet);
            }
            else
    ...
    void CComboUI::DoEvent(TEventUI& event)
    {
        ...
        if (event.Type == UIEVENT_BUTTONUP)
        {
            if ((m_uButtonState & UISTATE_CAPTURED) != 0)
            {
                ...
                // 				{
                // 
                // 					m_pManager->SendNotify(this,_T("ComboItemClick"),event.wParam,event.lParam);
                // 
                // 					m_SendItemClick = false;
                // 				}
                // 			}
    ...
    int CComboUI::GetStringItemHeight() const
    {
        return m_nStringItemHeight;
    }

    void CComboUI::SetStringItemHeight(int nHeight)
    {
        m_nStringItemHeight = nHeight;
    }
	...
	void CComboUI::SetIsSendItemClick(bool _istrue)
	{
		m_SendItemClick=_istrue;
	}

    bool CComboUI::IsOpenDropMenu() const
    {
