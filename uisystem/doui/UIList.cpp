...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
    ...
    m_pCurrentSelectList = new vtDWORD();
	...
	m_ListInfo.bMultiSele = false;
    ::ZeroMemory(&m_ListInfo.rcTextPadding, sizeof(m_ListInfo.rcTextPadding));
    ::ZeroMemory(&m_ListInfo.rcColumn, sizeof(m_ListInfo.rcColumn));

	///--
	m_isMultipleSele = false;
}

CListUI::~CListUI()
{
    if (m_pCurrentSelectList)
    {
        m_pCurrentSelectList->clear();
        delete m_pCurrentSelectList;
        m_pCurrentSelectList = NULL;
    }
}
...
void CListUI::SetEnabledChild(bool benable)
{
...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    case UIEVENT_KEYDOWN:
		...
		///--
		case VK_CONTROL:
			if(m_ListInfo.bMultiSele)
				m_isMultipleSele = true;
			return;
            }
        break;
	case UIEVENT_KEYUP:
...
bool CListUI::IsMultipleSele() const
{
	return m_isMultipleSele;
}

void CListUI::DoSubEvent(CControlUI* pcontrol,TSUBEventUI& event)
{

}

void CListUI::GetMultipleSeleList(vector<DWORD>& indexs)
{
    if (m_pCurrentSelectList)
    {
        indexs = (*m_pCurrentSelectList);
    }
}

bool CListUI::DelSelectMultipleItem(int iIndex, bool bTakeFocus)
{
	if( iIndex < 0 )
        return false;
    ...
    if (m_pCurrentSelectList)
    {
        for (vtIt = m_pCurrentSelectList->begin(); vtIt != m_pCurrentSelectList->end(); vtIt++)
        {
            if ((*vtIt) == (DWORD)iIndex)
            {
                m_pCurrentSelectList->erase(vtIt);
                break;
            }
        }
    }
...
bool CListUI::SelectMultipleItem(int iIndex, bool bTakeFocus)
{
	if(iIndex < 0)
        return false;
...
// 	}
    if (m_pCurrentSelectList)
    {
        ...
        m_pCurrentSelectList->push_back(iIndex);
        if (m_iCurSel > 0)
        {
            m_pCurrentSelectList->push_back(m_iCurSel);
            m_iCurSel = -1;
        }
    }
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
    if (m_pCurrentSelectList)
    {
        ...
        if ((!m_isMultipleSele) && (m_pCurrentSelectList->size() > 0))  //多选已经取消
        {
            for (vtIt = m_pCurrentSelectList->begin(); vtIt != m_pCurrentSelectList->end(); vtIt++)
            {
                pControl = GetItemAt((*vtIt));
                if (pControl)
                {
	...
	//m_vCursel.push_back(iIndex); //也加入队列
...
bool CListUI::ItemIsFolder(int iIndex)
{
	return false;
}

bool CListUI::ItemIsSelect(int iIndex)
{
	if (iIndex >= 0)
	{
		bool isFind = false;
...
void CListUI::SetEnableMultipleSele(bool enable)
{
	m_ListInfo.bMultiSele = enable;
}
...
void	CListUI::DoHeadItemEvent(TEventUI& event)
{
...
void	CListUI::SortItemWidth()
{
	std::vector<int> v_iWidth;
    std::vector<int>::iterator vtIt;
	...
	for( int it = 0; it < m_pHeader->GetCount(); it++ ) {
		v_iWidth.push_back(static_cast<CControlUI*>(m_pHeader->GetItemAt(it))->GetWidth());
	}
	...
	for(int i = 0; i < num; i++) {
		...
		if (p)
		{
			CControlUI* pListItem = static_cast<CControlUI*>(p->GetInterface(_T("ListContainerElement")));
			if( pListItem != NULL )
            {
                int n = 0;
                CStdString str;
                ...
                for (vtIt = v_iWidth.begin(), n = 0; vtIt != v_iWidth.end(); vtIt++, n++)
                {
                    str.Format(_T("%d"), n + 1);
                    pItem = this->GetManager()->FindSubControlByName(pListItem, str);
                    if (pItem)
                    {
                        pItem->SetFixedWidth((*vtIt));
                    }
                }
			}
		}
...
{
    ASSERT(m_pOwner);
    SetInset(CDuiRect(1, 1, 1, 1));	//设置Body区内边距，防止遮盖边框
}
...
{
	SetPadding(CDuiRect(1,1,1,1));	// 设置Header外边距，防止遮盖边框
}
...
void	CListHeaderUI::DoItemEvent(TEventUI& event)
{
	if (m_pParent)
	{
		static_cast<CListUI*>(m_pParent)->DoHeadItemEvent(event);
	}
}
...
void CListHeaderItemUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcscmp(pstrName, _T("seplinewidth")) == 0)
    {
        m_nSepLineWidth = _ttoi(pstrValue);
    }
    else if (_tcscmp(pstrName, _T("seplinecolor")) == 0)
    {
        LPTSTR pstr = NULL;
        if (*pstrValue == _T('#'))
        {
        ...
        m_dwSepLineColor = _tcstoul(pstrValue, &pstr, 16);
    }
    ...
    if( event.Type == UIEVENT_MOUSEMOVE )
    {
        if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
            ...
            if( rc.right - rc.left > GetMinWidth() ) {
				...
				{
					GetParent()->NeedParentUpdate();
					static_cast<CListHeaderUI*>(GetParent())->DoItemEvent(event);
				}
				m_pManager->SendNotify(this, _T("headermoving"));
            }
        }
        return;
    }
...
void CListHeaderItemUI::PaintStatusImage(HDC hDC)
{
    ...
    else if (m_nSepLineWidth > 0)
    {
        RECT rcSepLine = { 0 };

        rcSepLine.left = m_rcItem.right - m_nSepLineWidth;
        rcSepLine.right = m_rcItem.right - m_nSepLineWidth;
        rcSepLine.top = m_rcItem.top;
        rcSepLine.bottom = m_rcItem.bottom ;
        CRenderEngine::DrawLine(hDC, rcSepLine, m_nSepLineWidth, m_dwSepLineColor);
    }
}
...
void CListHeaderItemUI::PaintBorder(HDC hDC)
{
    if (m_nBorderSize > 0)
    {
        if (IsFocused())
        ...
        if (((m_uButtonState & UISTATE_HOT) || (m_uButtonState && UISTATE_FOCUSED)) && (m_dwFocusBorderColor != 0))
        {
            // 画获得焦点的
...
bool CListElementUI::SelectMultiple(bool bSelect)
{
	return true;
}
...
bool CListContainerElementUI::SelectMultiple(bool bSelect)
{
	if( !IsEnabled() ) return false;
	
	if( bSelect == m_bSelected )  //已被选择
	{
		m_bSelected = false;
		if(m_pOwner != NULL ) 
			m_pOwner->DelSelectMultipleItem(m_iIndex);
	}
	else
	{
		if (!m_pOwner->ItemIsFolder(m_iIndex))
		{
			m_bSelected = bSelect;
		}
		...
		{
			m_pOwner->SelectMultipleItem(m_iIndex);
		}
	}
	Invalidate();
...
void CListContainerElementUI::HideMultipleItem()
{
	if( !IsEnabled() ) return;
	if( m_pOwner != NULL ) m_pOwner->SelectItem(m_iIndex);
	Invalidate();
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_DBLCLICK )
    {
        if( IsEnabled() ) {
			HideMultipleItem();
            Activate();
			TSUBEventUI subevent;
			subevent.chKey = event.chKey;
			subevent.dwTimestamp = event.dwTimestamp;
			subevent.lParam = event.lParam;
			subevent.wParam = event.wParam;
			subevent.pSender = event.pSender;
			subevent.ptMouse = event.ptMouse;
			subevent.Type = event.Type;
			subevent.wKeyState = event.wKeyState;
			m_pOwner->DoSubEvent(this,subevent);
            Invalidate();
        }
        return;
    }
	...
	if( event.Type == UIEVENT_RBUTTONDOWN)
	{
		if( IsEnabled() )
        {
			...
			{
				vector<DWORD> indexs;
                vector<DWORD>::iterator vtIt;
                bool bhaveSelf = false;

                indexs.clear();
				m_pOwner->GetMultipleSeleList(indexs);
                for (vtIt = indexs.begin(); vtIt != indexs.end(); vtIt++)
				{
                    if ((*vtIt) == m_iIndex)
                    {
                        bhaveSelf = true;
                ...
                if (indexs.size() > (size_t)((bhaveSelf) ? 1 : 0))
                {
                    lparam = 1;
                }
			}
			m_pManager->SendNotify(this, _T("itemrclick"),0,lparam);
			Invalidate();
		}
		return;
	}
