...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    ...
    if( CPaintManagerUI::GetXMLResourceZip().IsEmpty() ) {
        sFile += pstrFilename;
        ...
        HZIP hz = NULL;
        if( CPaintManagerUI::IsCachedXMLResourceZip() ) hz = (HZIP)CPaintManagerUI::GetXMLResourceZipHandle();
        ...
        if( res != 0x00000000 && res != 0x00000600) {
            delete[] pByte;
            if( !CPaintManagerUI::IsCachedXMLResourceZip() ) CloseZip(hz);
