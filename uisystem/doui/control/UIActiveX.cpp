...
public:
    ...
    {
        TRACEOUT(m_pUnk->AddRef());
    }
    ~CActiveXEnum()
    {
        TRACEOUT(m_pUnk->Release());
    }
...
STDMETHODIMP CActiveXCtrl::OnInPlaceActivateEx(BOOL* pfNoRedraw, DWORD dwFlags)        
{
    ...
    if( m_pInPlaceObject != NULL/* && ! m_pOwner->IsDelayCreate() */) {
...
bool CActiveXUI::DoCreateControl()
{
    ...
    if( (dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) == 0 ) {
		Hr = m_pUnk->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, pOleClientSite, 0, m_pManager ? m_pManager->GetPaintWindow() : NULL, &m_rcItem);
	...
	BOOL bDummy = FALSE;
	Hr = m_pControl->OnInPlaceActivateEx(&bDummy, 0);
    return SUCCEEDED(Hr);
}
...
//////////////////////////////////////////////////////////////////////////
///Event
void CActiveXUI::InsertEvent(IActiveXEvent *ptr)
{
 	m_pIActiveXEvent = ptr;
}

HRESULT CActiveXUI::Event_Invoke(IID iid, CAtlStringW name, DISPPARAMS *  pDispParams)
{
	if (m_pIActiveXEvent)
	{
		m_pIActiveXEvent->DoActiveXEvent(iid,name,pDispParams);
	}
	return S_OK;
}
