...
namespace DuiLib {
    ...
    CControlUI::CControlUI() :
        ...
        m_bIsMouseTransparent(false),
        m_bKeyboardEnabled(true),
        m_bFloat(false),
        m_bSetPos(false),
        m_pEventTransparentUI(NULL),
        m_chShortcut('\0'),
        m_pTag(NULL),
        m_dwBackColor(0),
        m_dwBackColor2(0),
    ...
    void CControlUI::registerOwnerDraw(OwnerDraw *pUserDraw)
    {
    ...
    HWND CControlUI::GetParentWindow()
    {
    ...
    {
        if (m_dwDisabledBorderColor != dwBorderColor)
        {
    ...
    POINT CControlUI::GetXY() const
    {
        POINT pt = { GetX(),GetY() };
        return pt;
    }

    POINT CControlUI::GetRB() const
    {
        POINT pt = { m_rcItem.right,m_rcItem.bottom };
        return pt;
    }
    ...
    int	CControlUI::GetProportion()
    {
        return m_iProportion;
    }
    void	CControlUI::SetProportion(int proportion)
    {
        m_iProportion = proportion;
    }
    ...
    bool CControlUI::IsMouseTransparent() const
    {
        return m_bIsMouseTransparent;
    }

    void CControlUI::SetMouseTransparent(bool bIsTransparent /* = false */)
    {
        m_bIsMouseTransparent = bIsTransparent;
    }
    ...
    void CControlUI::SetEventTransparent(CControlUI *pTransparentUI)
    {
        m_pEventTransparentUI = pTransparentUI;
    }

    void CControlUI::Invalidate(RECT& pRc, BOOL b)
    {
        if (!IsVisible()) return;
        RECT invalidateRc = pRc;
        CControlUI* pParent = this;
        RECT rcTemp;
        RECT rcParent;
        while (pParent = pParent->GetParent())
        ...
        if (m_pManager != NULL) m_pManager->Invalidate(invalidateRc, b);
    }
    ...
    void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
    {
        ...
        else if (_tcscmp(pstrName, _T("proportion")) == 0) SetProportion(_ttoi(pstrValue));
        ...
        else if (_tcscmp(pstrName, _T("mousetransparent")) == 0) SetMouseTransparent(_tcscmp(pstrValue, _T("true")) == 0);
    ...
    void CControlUI::DoPaint(HDC hDC, const RECT& rcPaint)
    {
        ...
        if (m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0) {
            ...
            if (m_pUserDraw)
            {
                if (m_pUserDraw->OnOwnerPaint(hDC, roundClip.rcItem))
                    return;
            }
            else
                PaintText(hDC);
            ...
            if (m_pUserDraw)
            {
                if (m_pUserDraw->OnOwnerPaint(hDC, m_rcItem))
                    return;
            }
            else
                PaintText(hDC);
    ...
    bool CControlUI::EnableDropDrag(void)
    {
    ...
    bool CControlUI::RemoveDropDrag(void)
    {
        if (m_bUseDrop && GetManager() && GetManager()->EnableDropTarget())
        {
            GetManager()->RemoveDropUI(this);
            m_bUseDrop = FALSE;
            return true;
        }
        return false;
    ...
    HRESULT CControlUI::Event_Invoke(IID iid, CAtlStringW name, DISPPARAMS *  pDispParams)
    {
        return S_OK;
    }
